...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">问题上报管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>问题上报管理</span></li>
								<li class="breadcrumb-item active"><span>问题上报列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">问题上报列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										上报编号
									</label>
									<input type="text" id="shangbaobianhaoSearch" class="form-control form-control-sm" placeholder="请输入上报编号" aria-controls="tableId">
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shangbaobianhao')">上报编号<i id="shangbaobianhaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shangbaorenshu')">上报人数<i id="shangbaorenshuIcon" class="fa fa-sort"></i></th>
											<th>症状描述</th>
											<th onclick="sort('shangbaoshijian')">上报时间<i id="shangbaoshijianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#shangbaobianhaoSearch').val() != null && $('#shangbaobianhaoSearch').val() != ''){
				searchForm.shangbaobianhao ="%" +  $('#shangbaobianhaoSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("wentishangbao/page","GET",{
				page: pageIndex,
				limit: pageSize,
                sfsh: searchForm.sfsh,
				sort: sortColumn,
				order: sortOrder,
				shangbaobianhao : searchForm.shangbaobianhao,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var shangbaobianhaoCell  = document.createElement('td');

						 shangbaobianhaoCell.innerHTML = item.shangbaobianhao;

			row.appendChild(shangbaobianhaoCell);
			var shangbaorenshuCell  = document.createElement('td');

						 shangbaorenshuCell.innerHTML = item.shangbaorenshu;

			row.appendChild(shangbaorenshuCell);
			var zhengzhuangmiaoshuCell  = document.createElement('td');

						  if(item.zhengzhuangmiaoshu != null && item.zhengzhuangmiaoshu != "" && item.zhengzhuangmiaoshu.length >= 11){
			  	zhengzhuangmiaoshuCell.innerHTML = item.zhengzhuangmiaoshu.substring(0, 10) + "..."; 
			  }else{
			  	zhengzhuangmiaoshuCell.innerHTML = item.zhengzhuangmiaoshu;
			  }


			row.appendChild(zhengzhuangmiaoshuCell);
			var shangbaoshijianCell  = document.createElement('td');

						  shangbaoshijianCell.innerHTML = item.shangbaoshijian;


			row.appendChild(shangbaoshijianCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "问题处理"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'wentichuli','否','','')");
			if(crossBtnControl('问题处理') == true){
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("wentishangbao/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("wentishangbao/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("wentishangbao/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function getSum(colName){
		http("wentishangbao"+colName,"GET",{
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"wentishangbao");
