...
<body>
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>处理编号</label>
										<input id="chulibianhao" name="chulibianhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>上报人数</label>
										<input name="shangbaorenshu" id="shangbaorenshu" name="shangbaorenshu" class="form-control" placeholder="上报人数">
									</div>
									<div class="form-group col-md-6">
										<label>症状描述</label>
										<textarea  name="zhengzhuangmiaoshu" class="form-control" id="zhengzhuangmiaoshu" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>处理时间</label>
										<div id="chulishijianDate" class="input-append date form_datetime">
											<input id="chulishijian-input" name="chulishijian" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										...
										<input name="fengmian" id="fengmian-input" type="hidden"></div>
											...
											<div class="form-group  col-md-12" >
												<label>处理详情</label>
												<div><div class="upload"><input id="chulixiangqingupload" name="file" type="file">选择文件</div>
												<script id="chulixiangqingEditor" type="text/plain"></script>
	...
	<script>
		...
		function chulibianhaoUuid(){
			if($("#chulibianhao").val() == null || $("#chulibianhao").val() == "null" || $("#chulibianhao").val() == ""){
				$("#chulibianhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		...
		// 文件上传
		function upload(){
			$('#chulixiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('chulixiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
				...
				if(crossrefid && crossuserid) {
					...
					http("wentichuli/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("wentichuli/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function setContent(){
			if(ruleForm.chulixiangqing != null && ruleForm.chulixiangqing != 'null' && ruleForm.chulixiangqing != ''){
				var chulixiangqingMes = '' + ruleForm.chulixiangqing;
				var chulixiangqingUeditor = UE.getEditor('chulixiangqingEditor');
				chulixiangqingUeditor.ready(function() {
					chulixiangqingUeditor.setContent(chulixiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('chulixiangqingEditor').hasContents()){
				$('#chulixiangqing-input').attr('value',UE.getEditor('chulixiangqingEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("wentichuli/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#chulishijian-input").val(getCurDate());
		...
		function dataBind(){
			...
			$("#chulibianhao").val(ruleForm.chulibianhao);	
			$("#shangbaorenshu").val(ruleForm.shangbaorenshu);	
			$("#zhengzhuangmiaoshu").val(ruleForm.zhengzhuangmiaoshu);
			$("#chulishijian-input").val(ruleForm.chulishijian);
			$("#chulixiangqing").val(ruleForm.chulixiangqing);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.chulibianhao != null && res.data.chulibianhao != ''){

						$("#chulibianhao").val(res.data.chulibianhao);
						$("#chulibianhao").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.shangbaorenshu != null && res.data.shangbaorenshu != ''){

						$("#shangbaorenshu").val(res.data.shangbaorenshu);
						$("#shangbaorenshu").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.zhengzhuangmiaoshu != null && res.data.zhengzhuangmiaoshu != ''){

						$("#zhengzhuangmiaoshu").val(res.data.zhengzhuangmiaoshu);
					}
					...
					if(res.data != null && res.data != '' && res.data.chulixiangqing != null && res.data.chulixiangqing != ''){

						$("#chulixiangqing").val(res.data.chulixiangqing);
                        var chulixiangqingMes = '' + res.data.chulixiangqing;
                        var chulixiangqingUeditor = UE.getEditor('chulixiangqingEditor');
                        chulixiangqingUeditor.ready(function() {
                            chulixiangqingUeditor.setContent(chulixiangqingMes);
                        });
